...
class PostgresCredentialBroker(
    ...
    private fun createExtensions() {
        if (extensions != "none") {
            extensions.split(",").forEach {
                LOG.info("Creating extension \"$it\"")
                val query = "CREATE EXTENSION IF NOT EXISTS \"$it\""
    ...
    private fun initializePermissions() {
        val query = "revoke create on schema public from public"
        template.execute(query)
    }

    private fun credentialExists(credential: Credential): Boolean {
        val query = "select 1 from pg_roles where rolname='${credential.username}'"
    ...
    private fun updateCredential(credential: Credential) {
        ...
        query = "grant select, insert, update, delete on all tables in schema public to \"${credential.username}\""
        template.execute(query)
    }
