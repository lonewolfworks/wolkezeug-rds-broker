...
@Component
@Profile("mysql56")
open class Mysql56CredentialBroker(
    ...
    private val LOG = LoggerFactory.getLogger(Mysql56CredentialBroker::class.java)
    ...
    private fun grantAppAccess(credential: Credential) {
        val grantQuery = String.format("GRANT SELECT, INSERT, UPDATE, DELETE ON %s.* TO '%s'@'%%' IDENTIFIED BY '%s';", dataSourceProperties.databaseName, credential.username, credential.password)
    ...
    private fun grantAdminAccess(credential: Credential) {
        val grantQuery = String.format("GRANT ALL ON %s.* TO '%s'@'%%' IDENTIFIED BY '%s';", dataSourceProperties.databaseName, credential.username, credential.password)
        val flushQuery = "FLUSH PRIVILEGES;"
        template.execute(grantQuery)
        template.execute(flushQuery)
    }
